...
public class CourseSchedule {
    public static boolean canFinish(int numCourses, int[][] prerequisites) {
        if (numCourses < 0) {
            throw new IllegalArgumentException("Cannot visit a negative amount of courses!");
        }
        for (int[] prerequisite : prerequisites) {
            if (prerequisite.length == 0) {
                throw new IllegalArgumentException("Cannot provide empty arrays!");
            }
        ...
        if (graph.size() != numCourses) {
            throw new RuntimeException("Length of graph list is wrong!");
        }
        ...
        for (int i = 0; i < numCourses; i++) {
            ...
            if (!visited[i]) {
                throw new RuntimeException("A visited node is set as not visited!");
            }
            if (onPath[i]) {
                throw new RuntimeException("A node is still set as onPath after being visited!");
            }
        }
    ...
    private static boolean hasCycle(List<List<Integer>> graph, int current, boolean[] visited, boolean[] onPath) {
        ...
        for (int neighbor : graph.get(current)) {
            ...
            if (!visited[current]) {
                throw new RuntimeException("A node currently being visited is set as not visited!");
            }
            if (!onPath[current]) {
                throw new RuntimeException("A node is not set as onPath while being visited!");
            }
        }
